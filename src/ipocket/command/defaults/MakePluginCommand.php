...
class MakePluginCommand extends VanillaCommand{
	...
	public function __construct($name){
		...
		$this->setPermission("ipocket.command.makeplugin");
	}
	...
	public function execute(CommandSender $sender, $commandLabel, array $args){
		...
		$pharPath = Server::getInstance()->getPluginPath().DIRECTORY_SEPARATOR . "iPocket-iTX" . DIRECTORY_SEPARATOR . $description->getName()."_v".$description->getVersion().".phar";
		...
		if($description->getName() === "DevTools"){
			...
			$phar->setStub('<?php echo "iPocket-iTX plugin '.$description->getName() .' v'.$description->getVersion().'\nThis file has been generated using iPocket-iTX by QQ:1215714524 at '.date("r").'\n----------------\n";if(extension_loaded("phar")){$phar = new \Phar(__FILE__);foreach($phar->getMetadata() as $key => $value){echo ucfirst($key).": ".(is_array($value) ? implode(", ", $value):$value)."\n";}} __HALT_COMPILER();');
		}
		$phar->setSignatureAlgorithm(\Phar::SHA1);
		$reflection = new \ReflectionClass("ipocket\\plugin\\PluginBase");
